# Alterian.JA.Model.GetCurrentUserDefaultResponse
...
**Result** | **Object** | A user is a unique profile of permissions and linked to a set of user login credentials.              When someone logs in using those credentials, the permissions in the user profile are applied             Each user can be logged on multiple times simultaneously | [optional] 
